...
const nodeHttpAdapter: HttpAdapter = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            file_path: {
                type: "string",
                description:
                    "本地素材文件路径。SPEC: 允许绝对路径或相对路径（相对 process.cwd()），且必须是可读取的已有文件。",
            },
            filename: {
                type: "string",
...
async function resolveAccessToken(accessToken?: string, appId?: string, appSecret?: string): Promise<string> {
    if (accessToken) {
        return accessToken;
    }
...
function requireClientMethod<T extends keyof WechatExtendedClient>(name: T): NonNullable<WechatExtendedClient[T]> {
